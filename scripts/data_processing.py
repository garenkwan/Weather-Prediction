...
class weatherData:
    ...
    #     self.result = training_data(self.start_date, 
    #                                 self.end_date, 
    ...
    def get_data(date):
        ...
        for data_type in data_types:
            ...
            df_process = df_preprocessing(data, data_type)
            df_T = data_transpose(df_process, data_type)            
            df_T = data_fill_missing(df_T, date)
    ...
    def training_data(self, start_date, end_date, training_window, forecast):
        ...
        if days_per_window > 0 and days_per_window < days:
            df_list = get_data(date_range.pop(0))
            days_per_window -= 1
            days -= 1
            while days_per_window >= 0:
                extend_list = get_data(date_range.pop(0))
...
def data_transpose(df, data_type):
    df['timestamp'] = pd.to_datetime(df['date']) + pd.to_timedelta(df['time'], unit='h')
    df_T = df.loc[(df['data_type']==data_type), ['station_id', 'timestamp', 'value']]\
             .pivot(index='station_id', columns='timestamp', values='value')
    return df_T
    
def df_preprocessing(df, data_type):
    df['timestamp'] = pd.to_datetime(df['timestamp'])
    df['date'] = df['timestamp'].apply(lambda x: x.date)
    df['time'] = df['timestamp'].apply(lambda x: x.hour)
    groups = ['station_id', 'name', 'data_type', 'date', 'time', 'value']
...
#         testing for data_fill_missing()
#         test_date = pd.to_datetime(date) 
#         test_dates = [test_date + pd.to_timedelta(x, unit='h') for x in range(24)]
#         df_T = df_T.drop(test_dates, axis=1)
...
def training_data(start_date, end_date, training_window, forecast):
    '''
    Prepares a list of 3D arrays and predictor
    training_window: number of hours that should be used X
    forecast: number of hourst that should be used as Y
    ...
    while days > 0 or len(working_columns) >= window_size:
        while window_size <= len(working_columns):
            ...
            predict_data.append(np.sum(np.array(df_list[0][predict_column]), axis=1))
           
            working_columns.pop(0)
            # logging.info(working_columns)
            # logging.info(train_columns)
            # logging.info(predict_column)
...
if __name__ == '__main__':
    ...
    training, predict = training_data("2021-10-15", "2021-10-18", 25, 24)
    logging.info('\nTRAINING\nNumber of rows:\t{}\nSize of array:\t{}'.format(len(training), training[0].shape))
    logging.info('\nPREDICT\nNumber of rows:\t{}\nSize of array:\t{}'.format(len(predict), predict[0].shape))
    #print(training)
    #print(predict)