...
class weatherData:
    '''
    Creates iterable dataset that provides a 3D training matrix and 1D prediction array
    '''
    def __init__(self, 
    ...
    def __data_fill_missing__(self, df, date):
    ...
    def __data_transpose__(self, df, data_type):
    ...
    def __df_preprocessing__(self, df, data_type):
    ...
    def get_data(self, date):
        '''
        Helper function to get a list of 5 dataframes that are used for training
        The 5 dataframes are arranged in the following order:
        result[0] : 'environment/rainfall'
        result[1] : 'environment/relative-humidity'
        result[2] : 'environment/wind-direction'
        result[3] : 'environment/wind-speed'
        result[4] : 'environment/air-temperature'
        '''
        ...
        for data_type in data_types:
            ...
            df_process = self.__df_preprocessing__(data, data_type)
            df_T = self.__data_transpose__(df_process, data_type)            
            df_T = self.__data_fill_missing__(df_T, date)
    ...
    def training_data(self, 
        ...
        if days_per_window > 0 and days_per_window < days:
            ...
            #TODO: error handling when days_per_window > days
            df_list = self.get_data(date_range.pop(0))
        ...
        while days > 0 or len(working_columns) >= window_size:
            while window_size <= len(working_columns):
                ...
                predict_column = working_columns[training_window:window_size]
                training_data = np.array(
                                            (
                                            df_list[0][train_columns].to_numpy(),
                                            df_list[1][train_columns].to_numpy(),
                                            df_list[2][train_columns].to_numpy(),
                                            df_list[3][train_columns].to_numpy(),
                                            df_list[4][train_columns].to_numpy()
                                            )
                                        )
                predict_data = np.sum(df_list[0][predict_column].to_numpy(), axis=1)
